...
local osc_styles = {
    ...
    SeekbarFg = '{\\blur1\\bord1\\1c&H0000FF&}',
    VolumebarBg = '{\\blur0\\bord0\\1c&H999999&}',
    VolumebarFg = '{\\blur1\\bord1\\1c&HFFFFFF&}',
    Ctrl1 = '{\\blur0\\bord1\\1c&HFFFFFF&\\3c&H000000&\\fs52\\fnmaterial-design-iconic-font}',
    Ctrl2 = '{\\blur0\\bord1\\1c&HFFFFFF&\\3c&H000000&\\fs32\\fnmaterial-design-iconic-font}',
    Ctrl2Flip = '{\\blur0\\bord0\\1c&HFFFFFF&\\3c&HFFFFFF&\\fs24\\fnmaterial-design-iconic-font\\fry180',
    Ctrl3 = '{\\blur0\\bord1\\1c&HFFFFFF&\\3c&H000000&\\fs30\\fnmaterial-design-iconic-font}',
...
--
-- Overlay Menu System (Playlist / Audio / Subtitle)
--

local menu_key_bindings = {}

function menu_add_binding(key, name, fn, flags)
...
function menu_open(menu_type)
    ...
    menu_bar_anim_y = nil
    menu_bar_last_t = nil
    -- If opening audio/sub, pre-select current track
    if menu_type == 'audio' then
        local cur = get_track('audio')
        if cur > 0 then state.menu_selected = cur end
    elseif menu_type == 'sub' then
        local cur = get_track('sub')
        if cur > 0 then state.menu_selected = cur + 1 end  -- +1 because item 1 is "Off"
    elseif menu_type == 'playlist' then
        local pos = mp.get_property_number('playlist-pos', 0) + 1
        state.menu_selected = pos
    elseif menu_type == 'chapters' then
        local ch = mp.get_property_number('chapter', 0)
        state.menu_selected = ch + 1
    end
    -- Reset focus bar animation so it snaps to start position
    menu_bar_anim_y = nil
    menu_bar_last_t = nil
    -- Register keyboard bindings for the menu
    menu_key_bindings = {}
    menu_add_binding('up',    'menu-up',    function() menu_navigate(-1) end, 'repeatable')
    menu_add_binding('down',  'menu-down',  function() menu_navigate(1)  end, 'repeatable')
    menu_add_binding('enter', 'menu-enter', function() menu_confirm()    end)
    menu_add_binding('esc',   'menu-esc',   function() menu_close()      end)
    mp.add_forced_key_binding('mbtn_left', 'menu-click', function()
        menu_handle_click()
    end)
    menu_key_bindings[#menu_key_bindings + 1] = 'menu-click'
    request_tick()
end

function menu_close()
    -- Release bindings immediately
...
function menu_get_items()
    ...
    if state.menu_active == 'playlist' then
        ...
        for i, v in ipairs(playlist) do
            ...
            -- Format filename the same way the title bar does
            title = title:gsub('%.[^.]+$', '')  -- strip extension
            title = title:gsub('[%._]', ' ')     -- dots and underscores to spaces
            title = title:gsub('%s+', ' '):gsub('^%s+', ''):gsub('%s+$', '')
            -- Try to detect SxxExx pattern
            local show, season, episode, epname =
                title:match('^(.-)%s*[Ss](%d+)[Ee](%d+)%s*(.-)$')
            if show and season and episode then
                show = show:gsub('[%s%.%-]+$', ''):gsub('[%s%.%-]+', ' ')
                epname = (epname or '')
                    :gsub('1080p.*', ''):gsub('720p.*', '')
                    :gsub('BluRay.*', ''):gsub('WEB%-DL.*', '')
                    :gsub('WEBRip.*', ''):gsub('REMUX.*', '')
                    :gsub('x264.*', ''):gsub('x265.*', ''):gsub('HEVC.*', '')
                    :gsub('%s+$', ''):gsub('^%s+', '')
                -- Line 1: episode title (or show+code if no epname)
                -- Line 2: show name + episode code
                local line1 = epname ~= '' and epname or
                    string.format('%s • S%02dE%02d', show, tonumber(season), tonumber(episode))
                local line2 = epname ~= '' and
                    string.format('%s • S%02dE%02d', show, tonumber(season), tonumber(episode)) or ''
                items[i] = {label = line1, sublabel = line2,
                    current = (i == cur_pos), index = i}
            else
                -- fallback: clean up junk
                title = title
                    :gsub('1080p.*', ''):gsub('720p.*', '')
                    :gsub('BluRay.*', ''):gsub('WEB%-DL.*', '')
                    :gsub('%s+$', ''):gsub('^%s+', '')
                items[i] = {label = title, sublabel = '',
                    current = (i == cur_pos), index = i}
            end
        end
    elseif state.menu_active == 'audio' then
        if #tracks_osc.audio == 0 then
            ...
            for i, track in ipairs(tracks_osc.audio) do
                ...
                -- build a rich label: title already contains codec/channels/rate
                local label
                local sublabel = ''
                if title ~= '' then
                    -- Check if title has pattern "Name (Codec info...)"
    ...
    elseif state.menu_active == 'sub' then
        ...
        if #tracks_osc.sub == 0 then
            ...
            for i, track in ipairs(tracks_osc.sub) do
                ...
                local is_cur = (cur_sid ~= nil and track.id == cur_sid)
                items[i+1] = {label = label, current = is_cur, index = i, track_id = track.id}
            end
        end
...
function menu_handle_click()
    ...
    local menu_w = 740
    local row_h  = (is_pl or state.menu_active == 'audio') and 62 or 46
    local header_h = 56
    local pad_bot  = 18
    local y_safe_top = 8
...
function render_overlay_menu(ass)
    -- Handle fade animation
    if menu_closing then
        local speed = 1800  -- fast fade out
        local dt = 1/30
    ...
    elseif state.menu_active then
        local speed = 2200  -- fast fade in
        local dt = 1/30
        menu_alpha = math.min(255, menu_alpha + speed * dt)
        if menu_alpha < 255 then request_tick() end
    end
    ...
    -- ── Sizing ────────────────────────────────────────────────────────────────
    local menu_w    = 740           -- tighter width
    local pad_left  = 20            -- internal left padding
    local row_h     = (is_pl or state.menu_active == 'audio') and 62 or 46
    local header_h  = 56
    local pad_bot   = 18
    local bar_x     = pad_left + 10 -- red active bar x (unused, bar is at x0)
    local num_x     = 28            -- number centre: between bar and text
    local text_x    = pad_left + 50 -- text start
    ...
    -- dim:   original H55 (85), panel: original H4D (77)
    -- fade from transparent (HFF) to original value as menu_alpha goes 0→255
    local dim_a = string.format('%02X', math.floor(255 - (170 * eff_alpha / 255)))
    local pan_a = string.format('%02X', math.floor(255 - (178 * eff_alpha / 255)))
    ...
    -- ── Panel: solid black 70% opacity, sharp corners, no effects ────────────
    ass:new_event() ass:pos(x0, y0) ass:an(7)
    ass:append('{\\bord0\\blur0\\1c&H000000&\\1a&H'..pan_a..'&}')
    ass:draw_start()
    ass:rect_cw(0, 0, menu_w, menu_h)
    ass:draw_stop()
    ...
    ass:an(5)
    ass:append(string.format('{\\bord0\\blur0\\1c&HFFFFFF&\\1a&H00&\\fs20\\fn%s\\b1\\fsp3\\q2}', font))
    ...
    ass:append('{\\bord0\\blur0\\1c&H444444&\\1a&H00&}')
    ass:draw_start()
    ass:rect_cw(0, 0, menu_w, 1)
    ass:draw_stop()

    -- ── Animated focus bar: lerps to selected row ──────────────────────────
    do
        ...
        if bar_vis then
            if menu_bar_anim_y == nil then
                ...
                if math.abs(menu_bar_anim_y - tgt_y) > 0.5 then request_tick() end
            end
            menu_bar_last_t = now
            ass:new_event()
            ass:pos(x0, menu_bar_anim_y)
            ass:an(7)
            ass:append('{\\bord0\\blur0\\1c&H0000FF&\\1a&H00&}')
            ass:draw_start()
            ass:rect_cw(0, 0, 5, row_h - 10)
            ass:draw_stop()
        else
            menu_bar_anim_y = nil
        end
    ...
    for i = 1, vis do
        ...
        -- Row separator (subtle, skip first)
        if i > 1 then
            ass:new_event()
            ass:pos(x0 + text_x, ry)
            ass:an(7)
            ass:append('{\\bord0\\blur0\\1c&HFFFFFF&\\1a&HEE&}')
            ass:draw_start()
            ass:rect_cw(0, 0, menu_w - text_x - 20, 1)
            ass:draw_stop()
        end


        -- Row number — larger, tighter to text
        local ncol = (is_sel or is_cur) and '\\1c&HFFFFFF&' or '\\1c&H777777&'
        ass:new_event()
        ass:pos(x0 + num_x, ry + row_h / 2)
        ass:an(6)
        ass:append(string.format('{\\bord0\\blur0%s\\1a&H00&\\fs18\\fn%s\\b1\\q2}', ncol, font))
        ass:append(tostring(ai))

        if is_pl or (item.sublabel and item.sublabel ~= '') then
            -- ── Two-line layout for playlist ──────────────────────────────────
            local line1 = item.label    or ''
            local line2 = item.sublabel or ''
            local has2  = (line2 ~= '')

            -- Line 1: episode title
            local c1 = is_cur and '\\1c&HFFFFFF&' or (is_sel and '\\1c&HEEEEEE&' or '\\1c&HBBBBBB&')
            local b1 = (is_cur or is_sel) and '\\b1' or '\\b0'
            ass:new_event()
            ass:pos(x0 + text_x, ry + (has2 and (row_h / 2 - 2) or row_h / 2))
            ass:an(has2 and 1 or 4)
            ass:append(string.format('{\\bord0\\blur0%s\\1a&H00&\\fs20\\fn%s%s\\q2}', c1, font, b1))
            line1 = line1:gsub('^%s*%-%s*', '')
            line1 = line1:gsub('%s*%(+%s*$', '')
            if #line1 > 65 then line1 = line1:sub(1,63)..'\xe2\x80\xa6' end
            ass:append(line1)

            -- Line 2: show • SxxExx
            if has2 then
                local c2 = is_cur and '\\1c&HFFFFFF&' or (is_sel and '\\1c&HEEEEEE&' or '\\1c&HBBBBBB&')
                ass:new_event()
                ass:pos(x0 + text_x, ry + (row_h / 2 + 22))
                ass:an(1)
                ass:append(string.format('{\\bord0\\blur0%s\\1a&H00&\\fs17\\fn%s\\q2}', c2, font))
                ass:append(line2)
            end
        else
            -- ── Single line for audio/sub/chapters ───────────────────────────
            local tcol  = is_cur and '\\1c&HFFFFFF&' or (is_sel and '\\1c&HDDDDDD&' or '\\1c&H888888&')
            local tbold = (is_cur or is_sel) and '\\b1' or '\\b0'
            ass:new_event()
            ass:pos(x0 + text_x, ry + row_h / 2)
            ass:an(4)
            ass:append(string.format('{\\bord0\\blur0%s\\1a&H00&\\fs20\\fn%s%s\\q2}', tcol, font, tbold))
            local label = item.label or ''
            if #label > 78 then label = label:sub(1,76)..'\xe2\x80\xa6' end
            ass:append(label)
        end

        -- Active: red arrow right
        if is_cur then
            ass:new_event()
            ass:pos(x0 + menu_w - 18, ry + row_h / 2)
            ass:an(6)
            ass:append(string.format('{\\bord0\\blur0\\1c&H0000FF&\\1a&H00&\\fs20\\fn%s\\q2}', font))
            ass:append('\xe2\x96\xb6')
        end
    end
    ...
    if scroll > 0 then
        ass:new_event()
        ass:pos(x0 + menu_w / 2, y0 + header_h + 4)
        ass:an(8)
        ass:append(string.format('{\\bord0\\blur0\\1c&H777777&\\1a&H00&\\fs11\\fn%s\\q2}', font))
...
local UI_OFFSET_Y = -30
    ...
    lo.geometry = {x = 137, y = refY - 40 + UI_OFFSET_Y, an = 5, w = 24, h = 24}
    lo.style = osc_styles.Ctrl3
    ...
    -- tog_playlist: left side, between cy_sub/vol_ctrl and left edge area
    -- Sits at x=187, just after vol_ctrl (137), giving clear left-side grouping
    ...
    geo = { x = 25, y = refY - 132 + UI_OFFSET_Y, an = 1, w = osc_geo.w - 50, h = 48 }
    lo = add_layout('title')
    lo.geometry = geo
...
function render()
    ...
    -- Overlay menus (playlist / audio / sub)
